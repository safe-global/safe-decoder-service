...
class Abi(SqlQueryBase, TimeStampedSQLModel, table=True):
    ...
    async def get_abis_with_id_greater_than(cls, last_id: int) -> AsyncIterator[ABI]:
        ...
        Using an integer id cursor instead of a timestamp avoids the edge case
        where two ABIs share the same `created` timestamp and the newer one
        would be permanently skipped by a strict `>` timestamp comparison.
